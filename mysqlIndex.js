...
connection.connect(function(err){
    ...
    console.log('connection conntect succeed');
});
connection.query('use mynote');
...
app.get('/home', function (req, res) {
    ...
    //        });
    //    });
    connection.query('select * from note where author = ?',req.session.user.username,function(err,ret){
        if(err) {
            console.log(err);
            return res.redirect('/');
        }
...
app.get('/detail/:_id', function (req, res) {
    ...
    connection.query('select * from note where id = ?',req.params._id,function(err,ret){
        if(err) {
            console.log(err);
            return res.redirect('/');
        }
...
app.post('/update/:_id', function (req, res) {
    ...
    connection.query('select * from note where id = ?',req.params._id,function(err,ret) {
        ...
        if(ret)
        {
            ret.title=req.body.title;
            ret.tag=req.body.tag;
            ret.content=req.body.content;
            //art.save(function(err,doc){
            //    if(err)
            //    {
            //        console.log(err);
            ...
            //    });
            //});
            connection.query('update note set title=?,tag=?,content=? where id = ?',ret.title,ret.tag,ret.content,req.params._id,function(err,ret){
                if(err)
                {
                    console.log(err);
                    return res.redirect('/home');
...
app.get('/delete/:_id',function(req,res){
    ...
    connection.query('select * from note where id = ?',req.params._id,function(err,ret){
        ...
        if(ret)
        {
            connection.query('delete from note where id = ?',art._id,function(err,ret){
...
app.post('/post', function (req,res) {
    ...
    var insertSql = 'insert into note(title,author,tag,content) values(?,?,?,?)';
    var insertValue = [req.body.title,req.session.user.username,req.body.tag,req.body.content];
    connection.query(insertSql,insertValue,function(err,doc){
...
app.get('/profile/:username', function (req, res) {
    ...
    connection.query('select * from note where author=?',req.params.username,function(err,allNotes){
